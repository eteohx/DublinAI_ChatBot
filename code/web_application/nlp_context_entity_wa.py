...
# embeddings from 
df = pd.read_csv('G:/My Drive/DublinAI/Mini Projects/chatbot/the-movies-dataset/df_prep.csv',low_memory=False)
...
def last_context(session_df):
    try:
        a = session_df['context']
        return a.iloc[-1]
    except:
        return ' '

def last_entity(session_df):
    try:
        a = session_df['entity']
        return a.iloc[-1]
    except:
...
def get_context_entity(text, session_df): # NLP
    global insults
    global genres
    b = calculate_similarity(clean_text(text).lower(), vectorise(insults))
    if last_context(session_df) == 'start_conversation':
        ...
        if a.max()>0.5:
            ...
            entity = ''
    elif last_context(session_df) == 'information_genre':
