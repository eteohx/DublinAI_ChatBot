...
def movies_similar_to(entity,k,genre='all',method = 'collab',exclude_collection = True):
    ...
    movie_list = [''.join(x).title() for x in (temp_df['title'].iloc[idxes])]
    urls = [''.join(x) for x in (temp_df['imdbURL'].iloc[idxes])]
#    movies = ''
#    for i in movie_list:
#        movies = movies + ', <i>' + i.title() + '</i>'
#    movies = movies[2:]
#    movies = ', '.join([''.join(x) for x in (temp_df['title'].iloc[idxes])])
    return movie_list,urls,idxes
